...
our @options;
our %config;
my $iguard = "__OPTIONS_H_INCLUDED__";
my $prefix = "opt";
my $any_help_option;
...
sub print_impl {
	...
	for my $o (grep { defined $_->{'long'} } @options) {
		...
		if ($type->{'needs_val'}) {
			...
			&$assign_func($out, "\t\t\t", "\"--$o->{long}\"", "${prefix}_" . $o->{'name'}, "a");
			print_verify($out, "\t\t\t", "\"--$o->{long}\"", "${prefix}_" . $o->{'name'}, "a", $o->{'verify'}) if $o->{'verify'};
			...
			print $out "\t\t\t${prefix}_has_$name = 1;\n" if ($type->{'generate_has'});
			&$assign_func($out, "\t\t\t", "\"--$o->{long}\"", "${prefix}_" . $o->{'name'}, "argv[i]");
			print_verify($out, "\t\t\t", "\"--$o->{long}\"", "${prefix}_" . $o->{'name'}, "argv[i]", $o->{'verify'}) if $o->{'verify'};
			print_exit_call($out, "\t\t\t", $o->{'exit'}) if $o->{'exit'};
			print $out "\t\t\tcontinue;\n\t\t}\n";
			...
			print $out "\t\t${prefix}_has_$name = 1;\n" if ($type->{'generate_has'});
			&$assign_func($out, "\t\t\t", "\"--$o->{long}\"", "${prefix}_" . $o->{'name'});
			print_exit_call($out, "\t\t\t", $o->{'exit'}) if $o->{'exit'};
			print $out "\t\t\tcontinue;\n\t\t}\n";
		}
	}
	...
	for my $o (grep { defined $_->{'short'} } @options) {
		...
		if ($type->{'needs_val'}) {
			...
			print $out "\t\t\t\t\t${prefix}_has_$name = 1;\n" if ($type->{'generate_has'});
			&$assign_func($out, "\t\t\t\t\t","\"-$o->{short}\"", "${prefix}_" . $o->{'name'}, "(argv[i] + j + 1)");
			print_verify($out, "\t\t\t\t\t", "\"-$o->{short}\"", "${prefix}_" . $o->{'name'}, "(argv[i] + j + 1)", $o->{'verify'}) if $o->{'verify'};
			print_exit_call($out, "\t\t\t\t\t", $o->{'exit'}) if $o->{'exit'};
			print $out "\t\t\t\t\tbreak;\n";
			print $out "\t\t\t\t} else {\n\t\t\t\t\ti++;\n";
			print $out "\t\t\t\t\tif (!argv[i])\n\t\t\t\t\t\tdie_noValue_short('$o->{short}');\n" if ($type->{'needs_val'} eq "required");
			print $out "\t\t\t\t\t${prefix}_has_$name = 1;\n" if ($type->{'generate_has'});
			&$assign_func($out, "\t\t\t\t\t", "\"-$o->{short}\"", "${prefix}_" . $o->{'name'}, "argv[i]");
			print_verify($out, "\t\t\t\t\t", "\"-$o->{short}\"", "${prefix}_" . $o->{'name'}, "argv[i]", $o->{'verify'}) if $o->{'verify'};
			print_exit_call($out, "\t\t\t\t\t", $o->{'exit'}) if $o->{'exit'};
			print $out "\t\t\t\t\tbreak;\n\t\t\t\t}\n";
		} else {
			print $out "\t\t\t\t${prefix}_has_$name = 1;\n" if ($type->{'generate_has'});
